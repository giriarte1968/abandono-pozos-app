cat > restore_temporal.sh << 'EOF'
#!/bin/bash

# 1. Restaurar archivo de configuración
...
YAML
echo "✅ Archivo config/dynamicconfig/development-sql.yaml restaurado."

# 2. Parchear podman-compose.yml para agregar el volumen
# Usamos un reemplazo cuidadoso para insertar el volumen en el servicio temporal
if grep -q "config/dynamicconfig" podman-compose.yml; then
    echo "ℹ️ El podman-compose.yml ya parece tener la configuración."
else
    # Hacemos backup
    cp podman-compose.yml podman-compose.yml.bak
    
    # Sobrescribimos con la versión corregida (basada en el dump OCI + Fixes)
...
services:
  ...
  # Python Worker (OCI ARM64 compliant)
  worker:
    build:
      context: .
...
volumes:
  mysql_data:
    driver: local
YAML
    echo "✅ podman-compose.yml actualizado con montajes de volumen."
fi

echo ""
echo ">>> 3. Recreando Temporal (Apply changes)..."
...
while [ $current_attempt -le $max_attempts ]; do
    ...
    if [ $current_attempt -gt $max_attempts ]; then
        echo "⚠️ Advertencia: No vi 'service started' en logs, pero continuamos si el contenedor sigue vivo."
    fi
done

if ! podman ps | grep -q "braco_temporal_1"; then
    echo "❌ ERROR: Temporal murió de nuevo."
